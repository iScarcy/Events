...
export class RecurringEventsService {
  ...
  getEvents(eventsType:string):Observable<IEvents[]>{
    ...
    return this.httpEvents.get<IEventsDto[]>(baseApiUrl+eventsType).pipe(
      map(events => events.map(event => ({codEvent: event.eventID, type:event.eventType, date: event.dateEvent, description: event.description, typeID: event.eventTypeID, entityTypeID: event.entityTypeID, eventActionResult: EventActionResult.None})))
    );
  }
